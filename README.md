...
1. Rename the `cmd/databricks` and `pkg/databricks` directories to `cmd/YOURMIXIN` and
   `pkg/YOURMIXIN`.
1. Find the text `github.com/KiPIDesTAN/porter-databricks` in the repository and change it to
...
The project provides an implementation of the `databricksschema` command that is mostly functional. To fully implement this for your mixin, you simply need to provide a valid JSON schema. For reference, consult `pkg/databricks/schema/schema.json`.
